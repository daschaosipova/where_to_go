...
Сценарии использования к первому уроку курса Django Девман.

## 1. Артём проверяет сайт в действии

Ваш заказчик Артём хочет потестировать сайт — убедиться что всё работает и всё красиво. Проверять будет со своего ноутбука. У него уже есть доступ к админке, вы сообщили ему логин и пароль ранее. Также у него есть ссылка на репозиторий.
...
Артём попросил вас выдать доступ к админке контент-менеджеру Ирине. Она будет следить за наполнением сайта: обновлять текст и загружать картинки. В админке сайта она чаще всего будет выполнять две операции: добавлять новые локации на карту и редактировать старые. В обоих случаях алгоритм действий такой:
...
3. По инструкции наполнил БД тестовыми данными — выполняет команду `python manage.py load_place http://адрес/файла.json`. Команда скачивает JSON-файл и заносит локацию (название, координаты, описания и фотографии) в базу. Таких файлов может быть несколько — команда запускается для каждого.
